...
export const saveGeneratedOutput = async (req: AuthenticatedRequest & Request, res: Response) => {
    try {
        ...
        let project: { id: string; user_id: string }
        if (!projectId || projectId === 'new') {
            ...
            const { data: existing, error: existingErr } = await db.from('projects').select('*').eq('id', projectId).maybeSingle()
            if (existingErr || !existing) {
                ...
                if (REQUIRE_AUTH_PROJECTS && (existing as any).user_id !== ownerUserId) {
        ...
        // Insert artifacts
        const artifacts = files.map(f => ({ project_id: project.id, version_id: (version as any).id, path: f.path, content: f.content, sha256: f.sha256 }))
...
const pickRelevantFiles = (
    ...
    if (!sorted.length || (sorted[0] && (score.get(sorted[0].path) || 0) === 0)) {
        const fallback = files.filter(f => /App\.(tsx|jsx)$/.test(f.path) || /index\.(tsx|jsx|css)$/.test(f.path))
